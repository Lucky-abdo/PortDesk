...
# SECURITY_FILE and STUN_SERVERS stay in this file because they mutate at
...
# Shared executor — explicit pool size (os.cpu_count()*2+4 is Python default, we pin it)
_CPU_COUNT     = os.cpu_count() or 2
_EXECUTOR      = _futures.ThreadPoolExecutor(max_workers=min(32, _CPU_COUNT * 2 + 4),
                                              thread_name_prefix='portdesk-worker')
# Dedicated pool for input (mouse/keyboard). Kept separate from _EXECUTOR so a
# busy screen encoder / file op can never starve input handling (root cause of
# the "everything freezes after rapid clicks" bug). Small + serial-ish: input
# ops are fast and must run in order, but on their own threads they can't be
# blocked by streaming work.
_INPUT_EXECUTOR = _futures.ThreadPoolExecutor(max_workers=4,
...
FLAG_WATCH_ONLY   = cfg.watch_only
FLAG_NO_EXPLORER  = cfg.no_explorer
FLAG_NO_MOUSE     = cfg.no_mouse
FLAG_NO_KEYBOARD  = cfg.no_keyboard
FLAG_NO_WEBRTC    = cfg.no_webrtc
FLAG_NO_H264      = cfg.no_h264
FLAG_GREY         = cfg.grey
FLAG_SCALE        = cfg.scale
FLAG_UPLOAD_LIMIT = cfg.upload_limit
FLAG_BACKEND      = cfg.backend
FLAG_VERBOSE      = cfg.verbose
FLAG_NO_UPLOAD    = cfg.no_upload
FLAG_NO_DOWNLOAD  = cfg.no_download

# ── Runtime flag setter — updates cfg (primary) + scalar aliases (terminal/argparse) ──
def _set_flag(name, value):
    """Set a runtime flag. Updates the cfg object (primary source of truth)
    AND the module-level FLAG_* scalar aliases so terminal toggle works."""
    _map = {
...
STUN_SERVERS = [{'urls': 'stun:stun.l.google.com:19302'}, {'urls': 'stun:stun1.l.google.com:19302'}] if WEBRTC_AVAILABLE else []
# LAN-only by default — STUN helps when crossing NAT. Remove entries for pure LAN.
...
def _trigger_lockdown(reason=''):
    ...
    _mic_active      = _stream.mic_active
    ...
    while True:
        ...
        if cmd == 'y':
            ...
            if fname not in _BOOL_FLAGS:
                ...
                globals()[var_name] = bool_val
                setattr(_m, var_name, bool_val)
            ...
            for _fn, _vn in [
                ...
                _val = globals().get(_vn, getattr(_m, _vn, '?'))
    ...
    if platform.system() == 'Linux':
        ...
        if _sc:
            for _kv in ['net.ipv4.tcp_syncookies=1',
                ...
                try: subprocess.run([_sc, '-w', _kv], capture_output=True, timeout=3)
                except Exception: pass
...
class _SecurityHeadersMiddleware:
    _CSP = (
        ...
        # handlers and inline <script>. cdnjs is allowed so DOMPurify loads
        # (otherwise _safeHTML falls back to a weaker regex sanitizer).
...
# Backward-compat module-level aliases — reads work as-is.
# WRITES use _stream methods for atomicity.
screen_streaming     = False   # proxy: read _stream.screen_streaming
screen_thread        = None    # proxy: read _stream.screen_thread
_screen_last_error   = ''      # proxy: read _stream.screen_error
_current_stream_mode = ''      # proxy: read _stream.mode
_current_transport   = ''      # proxy: read _stream.transport

def _update_stream_status():
    """Print/update a live stream status line in the terminal."""
    global _current_stream_mode, _current_transport
    transport = _current_transport or 'WS'
    mode = _current_stream_mode or 'idle'
...
def screen_worker():
    global screen_streaming, _screen_last_error, _current_stream_mode, _current_transport
    _screen_last_error = ''
    ...
    use_h264 = (not cfg.no_h264) and _ffmpeg_encoder_ok and _ffmpeg_encoder is not None
    ...
    if use_h264:
        ...
        if not h264.start():
            use_h264 = False
            _current_stream_mode = 'JPEG (H264 failed — fallback)'
            _update_stream_status()
            _vprint("⚠ H264 streamer failed to start — falling back to JPEG", flush=True)
        else:
            _current_stream_mode = f'H264 via {_ffmpeg_encoder} ({target_w}x{target_h} @ {fps}fps)'
            _update_stream_status()
    ...
    if not use_h264:
        ...
        try:
            ...
            _TJPF_BGR = TJPF_BGR
            _current_stream_mode = 'JPEG (TurboJPEG)'
            _update_stream_status()
            _vprint("✅ screen: TurboJPEG active", flush=True)
        except Exception as _te:
            _current_stream_mode = 'JPEG (cv2 fallback)'
            _update_stream_status()
    ...
    # ── Unified capture backend loop ─────────────────────────────────────────
    with _stream_config_lock: cfg = stream_config.copy()
    mon_idx = max(0, cfg.get('monitor', 1) - 1)
    cap = _create_capture_backend(mon_idx)
    if cap is None:
        _screen_last_error = 'no capture backend available'
        return

    _current_stream_mode = cap.name + ' + ' + (_current_stream_mode.split(' + ')[-1] if _current_stream_mode else 'JPEG')
    _update_stream_status()
    _vprint(f"✅ screen: {cap.name} active", flush=True)

    _consec_err     = 0      # consecutive capture failures (busy-loop guard)
    _prev_raw_small = None   # tiny thumbnail of last RAW frame (idle detection)
    _prev_mouse     = (-1, -1)
    _RAW_DS         = 8       # thumbnail downscale for the cheap idle check
    ...
    try:
        while _stream.screen_streaming:
            try:
                ...
                with _stream_config_lock: cfg = stream_config.copy()
                fps          = max(1, cfg['fps'])
                # Adaptive pacing: pace to the sustainable fps the encoder can
                # actually keep up with (never above the user's chosen fps). This
                # turns wild 11<->30 oscillation into a steady rate on weak CPUs.
                with _pace_lock: _eff = _pace['eff_fps']
                frame_budget = 1.0 / max(1.0, min(float(fps), _eff))
                target_h     = cfg['height']

                arr, mon_info = cap.grab(max(0, cfg.get('monitor', 1) - 1))
                if arr is None:
                    # dxcam returns None when there is no NEW frame — that's an
                    # idle signal: nothing to do, just keep timing.
                    elapsed = time.perf_counter() - t0
                    if frame_budget - elapsed > 0.001: time.sleep(frame_budget - elapsed)
                    continue

                # ── Cheap idle check ─────────────────────────────────────────
                # On a tiny thumbnail of the RAW frame, decide if anything changed
                # (screen or cursor). If not, skip the expensive resize + cursor
                # draw + queue entirely — this is the main CPU saver while the
                # screen is static, without dropping the connection.
                ...
                if (not use_h264) and _small is not None and _prev_raw_small is not None \
                    ...
                    # Nothing changed — idle frame, skip the heavy pipeline.
                    elapsed = time.perf_counter() - t0
                    if frame_budget - elapsed > 0.001: time.sleep(frame_budget - elapsed)
                    continue
                _prev_raw_small = _small
                _prev_mouse     = (mx, my)
                _consec_err = 0   # successful capture resets the failure counter
                ...
                if use_h264:
                    ...
                    _loop_ref.call_soon(_put_frame, arr, cfg)
                ...
                _consec_err += 1
                # Exponential backoff so a persistent capture failure (device
                # unplugged, backend crash) does not spin a busy error-loop
                # spamming logs 10x/sec forever. Give up after N consecutive.
                ...
                if _consec_err >= 50:
                    _screen_last_error = f"capture failed {_consec_err}x — stopping: {e}"
        ...
        cap.close()
        _prev_arr = None  # release frame buffer reference
...
if WEBRTC_AVAILABLE:
    ...
    class ScreenCaptureTrack(MediaStreamTrack):
        ...
        def _get_cap(self, mon_idx):
            if self._cap is None or self._cap_mon != mon_idx:
                if self._cap:
                    try: self._cap.close()
                    except: pass
                self._cap     = _create_capture_backend(mon_idx)
                self._cap_mon = mon_idx
            return self._cap
        ...
        async def recv(self):
            ...
            frame_arr = await loop.run_in_executor(_EXECUTOR, self._capture)
        ...
        def _capture(self):
            ...
            try:
                ...
                target_h = cfg['height']
                mon_idx  = max(0, cfg.get('monitor', 1) - 1)
                cap      = self._get_cap(mon_idx)
                if cap is None: return None
                arr, mon_info = cap.grab(mon_idx)
                if arr is None: return None
                src_h, src_w = arr.shape[:2]
                orig_h, orig_w = src_h, src_w

                # M12/M13: Apply cfg.scale and cfg.grey in WebRTC capture
                if cfg.scale != 1.0:
                    arr = cv2.resize(arr, (int(src_w * cfg.scale), int(src_h * cfg.scale)),
                                     interpolation=cv2.INTER_AREA)
                    src_h, src_w = arr.shape[:2]
                if cfg.grey:
    ...
    try:
        ...
        if device_idx is None:
            ...
            if device_idx is None or (isinstance(device_idx, int) and device_idx < 0):
                ...
                _mic_active = False; _stream.mic_active = False; return
    ...
    try:
        ...
        if device_idx is None:
            ...
            if device_idx is None or (isinstance(device_idx, int) and device_idx < 0):
                ...
                audio_streaming = False; _stream.audio_streaming = False; return
        ...
        _vprint(f"❌ audio_worker: {e}", flush=True); audio_streaming = False; _stream.audio_streaming = False
...
def _save_scheduled(tasks):
    ...
    tmp = SCHED_FILE + '.tmp'
    with open(tmp, 'w') as f: json.dump(tasks, f, indent=2)
...
async def _dispatch(data: dict, ws):
    global _mic_active, _mic_worker_thread, audio_streaming  # mic/audio thread management
    ...
    if t == 'move':
        ...
        if changed and _stream.screen_streaming:
            ...
            screen_streaming   = _stream.screen_streaming
            screen_thread      = _stream.screen_thread
        ...
        if _mic_worker_thread and _mic_worker_thread.is_alive():
            _mic_active = False; _stream.mic_active = False
            try: _mic_queue.put_nowait(None)
            except: pass
            await asyncio.get_running_loop().run_in_executor(_EXECUTOR, _mic_worker_thread.join, 1.0)
        while not _mic_queue.empty():
            try: _mic_queue.get_nowait()
            except: break
        _mic_active = True; _stream.mic_active = True
        ...
        _stream.stop_mic()
        _mic_active        = _stream.mic_active
        _mic_worker_thread = _stream.mic_thread
        try: _mic_queue.put_nowait(None)
        except: pass
        ...
        if not (_mic_worker_thread and _mic_worker_thread.is_alive()):
            _mic_active = False; _stream.mic_active = False; return
        raw = data.get('data')
        if not raw: return
        try:
        ...
        try:
            ...
            return
        audio_streaming = True; _stream.audio_streaming = True
...
if __name__ == '__main__':
    ...
    _main_mod.cfg.watch_only   = globals()['cfg.watch_only']   = args.watch_only
    _main_mod.cfg.no_explorer  = globals()['cfg.no_explorer']  = args.no_explorer
    _main_mod.cfg.no_mouse     = globals()['cfg.no_mouse']     = args.no_mouse
    _main_mod.cfg.no_keyboard  = globals()['cfg.no_keyboard']  = args.no_keyboard
    _main_mod.cfg.no_upload    = globals()['cfg.no_upload']    = args.no_upload
    _main_mod.cfg.no_download  = globals()['cfg.no_download']  = args.no_download
    _main_mod.cfg.no_webrtc    = globals()['cfg.no_webrtc']    = args.no_webrtc
    _main_mod.cfg.no_h264      = globals()['cfg.no_h264']      = (not args.h264)
    _main_mod.cfg.grey         = globals()['cfg.grey']         = args.grey
    _main_mod.cfg.scale        = globals()['cfg.scale']        = args.scale
    _main_mod.cfg.backend      = globals()['cfg.backend']      = args.backend
    _main_mod.cfg.verbose      = globals()['cfg.verbose']      = args.verbose
    if args.upload_limit:
        globals()['cfg.upload_limit'] = _parse_size(args.upload_limit)
        set_max_body_size(globals()['cfg.upload_limit'])
    ...
    if args.ssl and not use_https:
        print('⚠ --ssl specified but cert/key unavailable — falling back to HTTP', flush=True)
