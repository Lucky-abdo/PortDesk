...
def _ensure_fonts_subsets() -> None:
    ...
    Font subsets are now committed to extras/fonts/ at build time. To
    regenerate them, run:
        python scripts/generate_font_subsets.py
    from the project root. The server serves them via the /static/fonts/
    route registered in pd_routes.py.
    """
    return
...
def _start_lifespan():
    ...
    # Font subsets are now committed to extras/fonts/ at build time.
    # _ensure_fonts_subsets() removed — see scripts/generate_font_subsets.py
    # to regenerate the .woff2 files if needed.
