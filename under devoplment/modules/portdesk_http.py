...
class Server:
    ...
    async def _handle_connection(self, reader, writer):
        ...
        REQUEST_TIMEOUT = 30.0  # Total budget for entire request lifecycle

        try:
            while True:
                # Wrap ENTIRE request processing in timeout
                try:
                ...
                if upgrade == 'websocket' and self._ws_handler and request.url.path == self._ws_path:
                    ...
                    if not await WebSocket.handshake(reader, writer, request.headers):
                        ...
                        tracer.debug(f'WS handshake failed for {client_host}: missing/invalid Sec-WebSocket-Key')
                        break
                    ...
                    try:
                        ...
                        from portdesk_trace import tracer
                        tracer.debug(f'WS handler error for {client_host}: {e!r}')
                        raise
                    finally:
                        try: ws._closed = True
                        except: pass
                ...
                try:
                    ...
                    await self._send_response(writer, JSONResponse({'error': 'request timeout'}, status_code=408))
                    break
