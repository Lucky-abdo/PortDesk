...
import json
import threading
from portdesk_trace import tracer, trace
...
_NEEDED_NAMES = [
    ...
    '_TokenBucket', '_ws_buckets', '_DataChannelClient',
...
# Trusted proxies for X-Forwarded-* headers (match SecurityMiddleware in SERVER.py)
...
# ════════════════════════════════════════════════════════════════════════════
# Pattern/PIN lockdown — SEPARATE tracking dicts
# ════════════════════════════════════════════════════════════════════════════
# Previously both PIN and pattern failures fed into the same _pin_fails /
# _pin_lockout dicts (keyed by IP). That meant failing the pattern burned
# PIN attempts and vice-versa, and lockouts on one method blocked the
# other — defeating the purpose of having a fallback auth method.
#
# We now keep the original _pin_fails / _pin_lockout / _pin_lockout_count
# for PIN attempts only, and add parallel _pattern_* dicts for pattern
# attempts. The /auth/pin_check handler selects the right family based
# on the resolved pin_type.
#
# These are imported from the server module (where they're defined as
# defaultdict(int) / {} / defaultdict(int)); we add the pattern-side
# counterparts here.
from collections import defaultdict as _defaultdict
_pattern_fails         = _defaultdict(int)   # ip → failed pattern attempts since last reset
_pattern_lockout       = {}                  # ip → unix-time when pattern lockout expires
_pattern_lockout_count = _defaultdict(int)   # ip → how many times pattern lockout has triggered

# ════════════════════════════════════════════════════════════════════════════
# Phase 6: macro execution tracking
# ════════════════════════════════════════════════════════════════════════════
# One entry per running macro: {name: {status, step, total, error, started_at}}
# Status transitions: 'running' → ('done' | 'error' | 'cancelled')
# Entries are cleared lazily when /macros/status is polled after completion.
#
# The client polls /macros/status?name=X every 500ms while a macro runs and
# uses the response to drive its progress UI ("Running... step 3/8").
_macro_running = {}
_macro_running_lock = threading.Lock()
...
async def websocket_endpoint(ws: WebSocket):
    ...
    try:
        ...
        if auth_data.get('type') == 'hmac_response':
            tracer.debug(f'WS {ip}: received HMAC response, verifying...')
            if _hmac_verify(_ws_challenge, auth_data.get('response', ''), ip):
                _hmac_authenticated = True
                tracer.debug(f'WS {ip}: HMAC verified OK')
                await ws.send_json({'type': 'hmac_ok', 'server_ts': int(time.time() * 1000)})
            else:
                tracer.debug(f'WS {ip}: HMAC verification FAILED')
                await ws.send_json({'type': 'hmac_fail'})
                await ws.close(1008)
                return
        else:
            tracer.debug(f'WS {ip}: invalid auth type {auth_data.get("type")}')
            await ws.close(1008)
            return
    except asyncio.TimeoutError:
        tracer.debug(f'WS {ip}: HMAC timeout')
        await ws.close(1008)
        return
    except Exception as e:
        tracer.debug(f'WS {ip}: HMAC error: {e}')
        await ws.close(1008)
        return

    # ── Set active client AFTER successful HMAC (atomic via _session) ─────
    tracer.debug(f'WS {ip}: claiming session...')
    ...
    if not ok:
        if reason == 'sleeping':
            ...
            if not ok:
                tracer.debug(f'WS {ip}: session claim failed after takeover')
                await ws.close(4001); return
            _log_event('session_takeover', ip=ip, detail='previous client was sleeping', severity='WARNING')
            tracer.debug(f'WS {ip}: took over sleeping session')
        else:
            tracer.debug(f'WS {ip}: session occupied ({reason}), closing')
            await ws.close(4001); return
    ...
    tracer.debug(f'WS {ip}: connected, manager notified')
    ...
    with _ws_pin_lock:
        if not _has_pin:
            ...
            tracer.debug(f'WS {ip}: no PIN required, verified')
        elif ip not in _ws_pin_verified:
            _ws_pin_verified[ip] = False
            tracer.debug(f'WS {ip}: PIN required, not verified')
...
async def webrtc_offer(request: Request):
    ...
    _webrtc_pcs.add(pc)
    _stream.transport = 'WebRTC'
    _stream.mode = 'WebRTC (LAN-only)'
    _update_stream_status()
    ...
    try:
        from modules.pd_pool_manager import apply_stream_config_to_capture
        apply_stream_config_to_capture()
...
async def webrtc_control_offer(request: Request):
    """WebRTC data channel endpoint — LAN-only WebRTC for control commands.
...
# ── Static font serving ──────────────────────────────────────────────────────
# Pre-subset .woff2 files live in extras/fonts/ and are served as static
# assets. This replaces the old runtime _ensure_fonts_subsets() code path
# which downloaded TTFs and ran pyftsubset at startup. The .woff2 files are
# now committed to the repo (regenerate via scripts/generate_font_subsets.py).
_FONT_FILES_ALLOWLIST = {
    'tajawal-regular-subset.woff2',
    'tajawal-bold-subset.woff2',
    'tajawal-extrabold-subset.woff2',
    'jetbrains-mono-subset.woff2',
    'jetbrains-mono-bold-subset.woff2',
    'fonts.css',
}

@app.get('/static/fonts/{font_file}')
async def serve_static_font(font_file: str):
    # Allowlist prevents path traversal (e.g. /static/fonts/../../etc/passwd).
    if font_file not in _FONT_FILES_ALLOWLIST:
        return JSONResponse({'error': 'not found'}, status_code=404)
    font_path = os.path.join(BASE_DIR, 'extras', 'fonts', font_file)
    if not os.path.isfile(font_path):
        return JSONResponse({'error': 'not found'}, status_code=404)
    media = 'font/woff2' if font_file.endswith('.woff2') else 'text/css'
    resp = FileResponse(font_path, media_type=media)
    # Fonts are immutable — cache aggressively (1 year)
...
async def screen_status(request: Request):
    ...
    return {
        ...
        'thread_alive': _stream.screen_thread is not None and _stream.screen_thread.is_alive(),
        'mss':          MSS_AVAILABLE,
        'dxcam':        DXCAM_AVAILABLE and platform.system() == 'Windows',
        'error':        _stream.screen_error,
        'stream_mode':  _stream.mode,
        'transport':    _stream.transport,
    }
...
async def flags_update(request: Request):
    ...
    for flag_name in runtime_flags:
        if flag_name in d:
            ...
            setattr(cfg, flag_name, val)
            updated.append(flag_name)
    if 'scale' in d and 0.1 <= float(d['scale']) <= 2.0:
        scale_val = float(d['scale'])
        setattr(cfg, 'scale', scale_val)
...
async def whitelist_remove_self(request: Request):
    ...
    with _sec_lock:
        ...
        # Clear ALL lockout entries for this IP (both pin and pattern)
        # Phase 4: lockout is now keyed by "{ip}:{pin_type}".
        for k in list(security.get("lockout", {}).keys()):
            if k == ip or k.startswith(f"{ip}:"):
                security["lockout"].pop(k, None)
        # Also clear legacy "ip" key form (back-compat with old security.json)
        security.get("lockout", {}).pop(ip, None)
        _pin_lockout.pop(ip, None)
        _pin_fails[ip] = 0
        _pin_lockout_count[ip] = 0
        _pattern_lockout.pop(ip, None)
        _pattern_fails[ip] = 0
        _pattern_lockout_count[ip] = 0
        _save_security()
    return {"ok": True}
        ...
        for d in _string.ascii_uppercase:
            ...
            if os.path.exists(path):
                try:
                    ...
                    # Catch ALL OSErrors — any error means the drive is
                    # inaccessible. This covers:
                    #   • winerror 21  (ERROR_NOT_READY) — card reader with no media
                    #   • winerror 31  (ERROR_DEVICE_NOT_CONNECTED) — disconnected drive
                    #   • winerror 1117 (ERROR_IO_DEVICE) — I/O failure
                    #   • winerror 5   (ERROR_ACCESS_DENIED) — no permission
                    #   • winerror 1   (ERROR_INVALID_FUNCTION) — unmounted network drive
                    #   • winerror 67  (ERROR_BAD_NET_NAME) — offline network share
                    #   • winerror 1326 (ERROR_LOGON_FAILURE) — network creds required
                    # Previously only a fixed list (21,31,1117,5) was skipped,
                    # which left I: drives (often mapped but offline) showing
                    # as accessible and then crashing os.listdir() downstream.
                    if cfg.verbose:
                        _vprint(f"Drive {path}: OSError winerror={getattr(e, 'winerror', None)} "
                                f"errno={e.errno}: {e}")
                    continue
                except Exception as e:
                    if cfg.verbose:
                        _vprint(f"Drive {path}: non-OSError {type(e).__name__}: {e}")
                    continue
        return drives
...
async def explorer_download(request: Request):
    ...
    # ── Streaming ZIP for large folder downloads ──────────────────────────────
    # Uses a background thread to write ZIP chunks to a queue, allowing
    # immediate streaming response while ZIP is being created.
    import zipfile, tempfile, threading, queue
    def _streaming_zip_generator(folder):
                    ...
                    for root, _, files in os.walk(folder):
                        for fname in files:
                            ...
                            try:
                                ...
                                pass
                # After ZIP is written, stream it in chunks
        ...
        writer_thread = threading.Thread(target=_zip_writer, daemon=True)
        writer_thread.start()
    ...
    return StreamingResponse(
        _streaming_zip_generator(real), media_type='application/zip',
...
async def explorer_download_multi(request: Request):
    ...
    from modules.pd_pool_manager import get_io_executor
    from modules.pd_download_hybrid import explorer_download_multi_hybrid
    executor = get_io_executor()
    return await explorer_download_multi_hybrid(request, paths, executor)
...
async def macros_run(request: Request):
    ...
    # Phase 6: refuse to start a second instance of the same macro while
    # one is already running. Without this, rapid double-tap on the Run
    # button spawns two threads stomping on the same pyautogui state.
    with _macro_running_lock:
        cur = _macro_running.get(name)
        if cur and cur.get('status') == 'running':
            return JSONResponse({'error': 'already running', 'running': True,
                                 'progress': cur.get('step', 0),
                                 'total':    cur.get('total', len(steps))},
                                status_code=409)
        _macro_running[name] = {
            'status':     'running',
            'step':       0,
    ...
    def _run():
        ...
        if cfg.watch_only:
            with _macro_running_lock:
                if name in _macro_running:
                    _macro_running[name]['status'] = 'cancelled'
            return
        deadline = time.time() + MACRO_TIMEOUT
        for i, step in enumerate(steps):
            # Phase 6: check the shared status flag — if the user (or a
            # timeout) flipped it to 'cancelled', stop early. This makes
            # /macros/cancel (if added later) trivially implementable.
            with _macro_running_lock:
                if _macro_running.get(name, {}).get('status') != 'running':
                    return
                _macro_running[name]['step'] = i + 1
            ...
            try:
                ...
                _vprint(f"macro step error: {e}", flush=True)
                # Phase 6: record the error and bail — don't silently
                # continue and report 'done' at the end.
                with _macro_running_lock:
                    if name in _macro_running:
                        _macro_running[name]['status'] = 'error'
                        _macro_running[name]['error']  = str(e)
                return
        with _macro_running_lock:
            if name in _macro_running:
                _macro_running[name]['status'] = 'done'
                _macro_running[name]['step']   = len(steps)
    threading.Thread(target=_run, daemon=True).start()
    return {'ok': True, 'started': True, 'total': len(steps)}


@app.get('/macros/status')
async def macros_status(request: Request):
    """Phase 6: poll macro execution state.

    Query: ?name=<macro name>
    Returns:
      {running: bool, progress: int, total: int, error: str|null, status: str}

    status is one of: 'running' | 'done' | 'error' | 'cancelled' | 'unknown'

    The client polls this every 500ms while a macro runs. Once status is
    terminal (done/error/cancelled), the entry is cleared so the next
    run of the same macro starts clean.
    """
    name = request.query_params.get('name', '')
    if not name:
        return JSONResponse({'error': 'name required'}, status_code=400)
    with _macro_running_lock:
        info = _macro_running.get(name)
        if not info:
        ...
        # If terminal, clear the entry — the next /macros/run starts fresh.
        if info['status'] in ('done', 'error', 'cancelled'):
            _macro_running.pop(name, None)
        return snapshot
...
async def monitors_list(request: Request):
    ...
    if not MSS_AVAILABLE or _mss is None:
        return JSONResponse({'error': 'Monitor detection unavailable'}, status_code=503)
    try:
...
async def tasks_kill(request: Request):
    ...
    # Get process info for safety checks — Phase 5: psutil-backed _get_proc_info
    # now also returns suspicious/verified/type flags used to enrich the response.
    proc_info = _get_proc_info(pid_int)
    proc_name = proc_info.get('name', '').lower()
    proc_suspicious = bool(proc_info.get('suspicious', False))
    ...
    if ok:
        _log_event('task_kill', f'pid={pid} name={proc_name} suspicious={proc_suspicious}')
        # Phase 5: include the suspicious flag so the client can show a
        # "killed a suspected virus" toast vs a normal "stopped" toast.
        return {'ok': True, 'suspicious': proc_suspicious, 'name': proc_info.get('name', '')}
    else:
        code = 404 if err == 'process not found' else 403 if err == 'access denied' else 500
        return JSONResponse({'error': err, 'suspicious': proc_suspicious}, status_code=code)
...
async def auth_pin_check(request: Request):
    ...
    # ── Phase 4: separate lockout tracking per pin_type ──────────────────────
    # Pattern attempts feed _pattern_* dicts; PIN attempts feed _pin_* dicts.
    # A pattern lockout no longer blocks PIN login (and vice-versa), so the
    # user can always fall back to the alternate method.
    if pin_type == 'pattern':
        fails_dict         = _pattern_fails
        lockout_dict       = _pattern_lockout
        lockout_count_dict = _pattern_lockout_count
    else:
        fails_dict         = _pin_fails
        lockout_dict       = _pin_lockout
        lockout_count_dict = _pin_lockout_count

    if ip in lockout_dict and now < lockout_dict[ip]:
        rem = int(lockout_dict[ip] - now)
        return JSONResponse({'error': f'Blocked. Wait {rem} seconds'}, status_code=429)

    if d.get('_probe'):
        # Probe is now richer — also report the lockout state of BOTH methods
        # so the client can show "Try PIN instead" / "Try pattern instead"
        # buttons correctly. See /auth/lockdown_status for the canonical
        # source of this information.
        return {
            'server_pin': bool(client_hash),
            'pin_type':   pin_type,
            'pattern_locked': bool(ip in _pattern_lockout and now < _pattern_lockout[ip]),
            'pin_locked':     bool(ip in _pin_lockout     and now < _pin_lockout[ip]),
        }
    ...
    if ok:
        with _ws_pin_lock:
            _ws_pin_verified[ip] = True
        with _sec_lock:
            # Clear ONLY the lockout state for the method that just succeeded.
            # The other method's lockout (if any) is preserved so a successful
            # pattern login doesn't wipe an active PIN brute-force lockout.
            fails_dict[ip] = 0
            lockout_dict.pop(ip, None)
            # Persist: drop only THIS pin_type's entry from security["lockout"].
            lockout_key = f"{ip}:{pin_type}"
            security.get("lockout", {}).pop(lockout_key, None)
            _save_security()
        _log_event('pin_success', f'type={pin_type}', ip=ip)
        return {'ok': True}

    with _sec_lock:
        fails_dict[ip] += 1
        _log_event('pin_fail', f'attempt={fails_dict[ip]} type={pin_type}', ip=ip)
        if fails_dict[ip] >= PIN_MAX_TRIES:
            step     = lockout_count_dict[ip]
            duration = PIN_LOCKOUT_STEPS[min(step, len(PIN_LOCKOUT_STEPS) - 1)]
            lockout_count_dict[ip] += 1
            lockout_dict[ip] = time.time() + duration
            fails_dict[ip]   = 0
            # Persist lockout state with pin_type tag so a server restart
            # restores the correct lockout per method.
            if "lockout" not in security: security["lockout"] = {}
            lockout_key = f"{ip}:{pin_type}"
            security["lockout"][lockout_key] = {
                "until": time.time() + duration,
                "count": int(lockout_count_dict[ip]),
                "pin_type": pin_type,
            }
            _save_security()
            return JSONResponse({'error': f'Locked for {duration} seconds due to multiple failed attempts'},
                                status_code=429)
        remaining = PIN_MAX_TRIES - fails_dict[ip]
    return {'ok': False, 'remaining': remaining, 'pin_type': pin_type}


@app.get('/auth/lockdown_status')
async def auth_lockdown_status(request: Request):
    """Return the per-method lockout state for the calling IP.

    Used by CLIENT.html on connect to decide which "Try X instead" button
    to show. Pattern and PIN lockouts are tracked independently — see the
    Phase 4 comment block above /auth/pin_check.
    """
    ip  = request.client.host
    now = time.time()
    pattern_rem = 0
    pin_rem     = 0
    if ip in _pattern_lockout and now < _pattern_lockout[ip]:
        pattern_rem = int(_pattern_lockout[ip] - now)
    if ip in _pin_lockout and now < _pin_lockout[ip]:
        pin_rem = int(_pin_lockout[ip] - now)
    return {
        'pattern_locked':     pattern_rem > 0,
        'pattern_remaining':  pattern_rem,
        'pin_locked':         pin_rem > 0,
        'pin_remaining':      pin_rem,
    }
...
async def auth_get_client_key(request: Request):
    ...
    # Support X-Forwarded-Proto from trusted reverse proxies (match SecurityMiddleware).
    scheme = request.url.scheme
    if ip not in ('127.0.0.1', '::1', 'localhost'):
        xfp = request.headers.get('x-forwarded-proto', '')
        if xfp and ip in _TRUSTED_PROXIES:
...
async def log_export(request: Request):
    ...
    try:
        ...
        with _tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False, encoding='utf-8') as tmp:
            tmp.write(export_json)
            tmp_path = tmp.name
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        try:
            return FileResponse(tmp_path, filename=f'portdesk_audit_{timestamp}.json', media_type='application/json')
        except Exception:
            try:
                os.unlink(tmp_path)
            except:
