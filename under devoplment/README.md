...
3. Run `SERVER.py` — the IP shown in the terminal will be your hotspot IP
...
| 2 | WebRTC (LAN-only, no STUN) | Different networks — auto-fallback |

Layer 2 uses WebRTC peer-to-peer for LAN-to-LAN via VPN tunnels (Tailscale, NetBird). No external STUN servers — works on same subnet or over overlay networks. If it fails, see the options below.
...
python SERVER.py   # auto-detects the certificates and starts HTTPS
```
...
| File | Description |
|---|---|
| `SERVER.py` | The HTTP/WebSocket server — run this on your PC |
| `CLIENT.html` | The phone UI — served automatically by the server |
...
MIT License — Copyright (c) 2024 Lucky-abdo. See source files for license headers.
