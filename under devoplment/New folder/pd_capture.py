...
class _WfRecorderCaptureBackend(_CaptureBase):
    ...
    def __init__(self, mon_idx=0):
        ...
        try:
            ...
            # Output format: x,y widthxheight
            geo = r.stdout.strip()
            # Parse geometry: "x,y widthxheight" or just "widthxheight"
            if ' ' in geo:
        ...
        self._mon = {'left': x, 'top': y, 'width': w, 'height': h}
        self._fsize = w * h * 3
        # wf-recorder: rawvideo, bgr24, output to stdout
        cmd = [
    ...
    def grab(self, mon_idx=0):
        try:
            ...
            if r.returncode or not r.stdout:
                return None, {}
            # Parse PPM header exactly: P6\nWIDTH HEIGHT\nMAXVAL\nDATA
            # Header is exactly 3 lines (no comments in grim PPM output)
...
def _build_capture_chain(mon_idx=0):
    ...
    if _sys == 'Windows':
        ...
        if _wayland:
            # Priority: wf-recorder (DMA-BUF, zero-copy) → grim (exact-bytes PPM) → mss (fallback)
